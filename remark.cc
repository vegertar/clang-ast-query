...
public:
  ...
  void run(const MatchFinder::MatchResult &result) override {
    ...
    } else if (auto p = result.Nodes.getNodeAs<FieldDecl>("field")) {
      obj = p;
      value_type = p->getType().getTypePtr();
...
std::unique_ptr<ASTConsumer> make_ast_consumer(std::unique_ptr<raw_ostream> os,
                                               CompilerInstance &compiler,
                                               std::string_view in_file) {
